namespace OnlineShopWeb.Migrations
{
    ...
    public partial class DropUsersTable : DbMigration
    {
        ...
        {
            // Xóa dữ liệu liên quan trước khi xóa ràng buộc
            Sql("DELETE FROM dbo.UserProducts WHERE UserId IS NOT NULL");
            Sql("DELETE FROM dbo.Orders WHERE CustomerId IS NOT NULL");
            Sql("DELETE FROM dbo.Carts WHERE CustomerId IS NOT NULL");
